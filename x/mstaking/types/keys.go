...
var (
	...
	ValidatorWhitelistKey = []byte{0x12} // prefix for each key to a validator index, sorted by power
...
// GetValidatorWhitelistKey returns a key prefix for indexing ValidatorWhitelist.
func GetValidatorWhitelistKey(valAddr sdk.ValAddress) []byte {
	return append(ValidatorWhitelistKey, address.MustLengthPrefix(valAddr)...)
}

// AddressFromValidatorWhitelistKey creates the validator operator address from ValidatorWhitelistKey
func AddressFromValidatorWhitelistKey(key []byte) []byte {
	return key[2:] // remove prefix bytes and address length
}
