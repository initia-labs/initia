...
func (sh *Helper) CreateValidator(addr sdk.ValAddress, pk cryptotypes.PubKey, stakeAmount math.Int, ok bool) {
	coins := sdk.NewCoins(sdk.NewCoin(sh.Denom, stakeAmount))
	sh.createValidator(addr, pk, coins, ok)
}
...
func (sh *Helper) CreateValidatorWithValPower(addr sdk.ValAddress, pk cryptotypes.PubKey, valPower int64, ok bool) math.Int {
	amount := sh.k.VotingPowerFromConsensusPower(sh.Ctx, valPower)
	coins := sdk.NewCoins(sdk.NewCoin(sh.Denom, amount))
	sh.createValidator(addr, pk, coins, ok)
	return amount
}
...
func (sh *Helper) CreateValidatorMsg(addr sdk.ValAddress, pk cryptotypes.PubKey, stakeAmount math.Int) *stakingtypes.MsgCreateValidator {
	coins := sdk.NewCoins(sdk.NewCoin(sh.Denom, stakeAmount))
	msg, err := stakingtypes.NewMsgCreateValidator(addr, pk, coins, stakingtypes.Description{}, sh.Commission)
	require.NoError(sh.t, err)
	return msg
}

func (sh *Helper) createValidator(addr sdk.ValAddress, pk cryptotypes.PubKey, coins sdk.Coins, ok bool) {
	msg, err := stakingtypes.NewMsgCreateValidator(addr, pk, coins, stakingtypes.Description{}, sh.Commission)
	require.NoError(sh.t, err)
	sh.Handle(msg, ok)
}
...
func (sh *Helper) Delegate(delegator sdk.AccAddress, val sdk.ValAddress, amount sdk.Coins) {
	msg := stakingtypes.NewMsgDelegate(delegator, val, amount)
	sh.Handle(msg, true)
}
...
func (sh *Helper) DelegateWithPower(delegator sdk.AccAddress, val sdk.ValAddress, power int64) {
	coins := sdk.NewCoins(sdk.NewCoin(sh.Denom, sh.k.VotingPowerFromConsensusPower(sh.Ctx, power)))
	msg := stakingtypes.NewMsgDelegate(delegator, val, coins)
	sh.Handle(msg, true)
}

// Undelegate calls handler to unbound some stake from a validator.
func (sh *Helper) Undelegate(delegator sdk.AccAddress, val sdk.ValAddress, amount sdk.Coins, ok bool) *sdk.Result {
	msg := stakingtypes.NewMsgUndelegate(delegator, val, amount)
