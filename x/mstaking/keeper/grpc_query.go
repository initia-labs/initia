...
func (q Querier) Validators(ctx context.Context, req *types.QueryValidatorsRequest) (*types.QueryValidatorsResponse, error) {
	...
	if req.Status == types.Bonded.String() {
		...
		validators, pageRes, err = query.CollectionFilteredPaginate(ctx, q.ValidatorsByConsPowerIndex, req.Pagination, func(key collections.Pair[int64, []byte], _ bool) (include bool, err error) {
			valAddr := key.K2()
...
func (q Querier) RedelegationsOfDelegator(ctx context.Context, req *types.QueryRedelegationsRequest) (*types.QueryRedelegationsResponse, error) {
	return q.Redelegations(ctx, req)
}
