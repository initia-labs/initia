...
func (q Querier) Validators(ctx context.Context, req *types.QueryValidatorsRequest) (*types.QueryValidatorsResponse, error) {
	...
	if req.Status == types.Bonded.String() {
		validators, pageRes, err = query.CollectionPaginate(ctx, q.Keeper.ValidatorsByPowerIndex, req.Pagination, func(key collections.Pair[int64, []byte], _ bool) (types.Validator, error) {
			valAddr := key.K2()
