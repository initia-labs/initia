...
func (suite *KeeperTestSuite) sendFetchPrice(
	...
	msgTransfer := consumertypes.NewMsgFetchPrice(
...
func (suite *KeeperTestSuite) validateFetchedPrices(
	fromEndpoint, toEndpoint *ibctesting.Endpoint,
	expectedCurrencyPrices []types.CurrencyPrice,
) {
	ctx := fromEndpoint.Chain.GetContext()
	ck := fromEndpoint.Chain.GetInitiaApp().FetchPriceConsumerKeeper

	for _, currencyPrice := range expectedCurrencyPrices {
		price, err := ck.Prices.Get(ctx, currencyPrice.CurrencyId)
		suite.NoError(err)
		suite.Equal(currencyPrice.QuotePrice.Price, price.Price)
	}
}
