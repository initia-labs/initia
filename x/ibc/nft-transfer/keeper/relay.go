...
func (k Keeper) sendNftTransfer(
	...
	if types.SenderChainIsSource(sourcePort, sourceChannel, fullClassIdPath) {
		labels = append(labels, telemetry.NewLabel(ibcmetrics.LabelSource, "true"))
		...
		labels = append(labels, telemetry.NewLabel(ibcmetrics.LabelSource, "false"))
...
func (k Keeper) OnRecvPacket(ctx sdk.Context, packet channeltypes.Packet, data types.NonFungibleTokenPacketData) error {
	...
	labels := []metrics.Label{
		telemetry.NewLabel(ibcmetrics.LabelSourcePort, packet.GetSourcePort()),
		telemetry.NewLabel(ibcmetrics.LabelSourceChannel, packet.GetSourceChannel()),
	}
	...
	if types.ReceiverChainIsSource(packet.GetSourcePort(), packet.GetSourceChannel(), data.ClassId) {
		...
		defer func() {
			...
			telemetry.IncrCounterWithLabels(
				...
				1,
				append(
					labels, telemetry.NewLabel(ibcmetrics.LabelSource, "true"),
				),
			)
		}()
	...
	defer func() {
		...
		telemetry.IncrCounterWithLabels(
			...
			1,
			append(
				labels, telemetry.NewLabel(ibcmetrics.LabelSource, "false"),
			),
		)
	}()
