...
func (endpoint *Endpoint) QueryProof(key []byte) ([]byte, clienttypes.Height) {
	...
	lightClientModule, err := endpoint.Counterparty.Chain.GetLightClientModule(endpoint.Counterparty.Chain.GetContext())
	require.NoError(endpoint.Counterparty.Chain.T, err)

	consensusHeight := lightClientModule.LatestHeight(endpoint.Counterparty.Chain.GetContext(), endpoint.Counterparty.ClientID).(clienttypes.Height)
	// query proof on the counterparty using the latest height of the IBC client
	return endpoint.QueryProofAtHeight(key, consensusHeight.GetRevisionHeight())
}
...
func (endpoint *Endpoint) ConnOpenTry() error {
	...
	msg := connectiontypes.NewMsgConnectionOpenTry(
		...
		endpoint.Counterparty.Chain.GetPrefix(), []*connectiontypes.Version{ConnectionVersion}, endpoint.ConnectionConfig.DelayPeriod,
