...
func NewTestChainWithValSet(t *testing.T, coord *Coordinator, chainID string, valSet *cmttypes.ValidatorSet, signers map[string]cmttypes.PrivValidator) *TestChain {
...
// GetInitiaApp returns the InitiaApp to allow usage of non-interface fields.
// CONTRACT: This function should not be called by third parties implementing
// their own InitiaApp.
func (chain *TestChain) GetInitiaApp() *initiaapp.InitiaApp {
	app, ok := chain.App.(*initiaapp.InitiaApp)
...
func (chain *TestChain) GetClientLatestHeight(clientID string) exported.Height {
	return chain.App.GetIBCKeeper().ClientKeeper.GetClientLatestHeight(chain.GetContext(), clientID)
}
