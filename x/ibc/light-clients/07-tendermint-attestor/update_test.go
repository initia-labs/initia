...
func (suite *TMAttestorTestSuite) TestVerifyHeader() {
	var (
	...
	testCases := []struct {
		...
		},
		{
			...
			malleate: func() {
				...
				header = suite.chainB.CreateTMAttestorClientHeader(suite.chainB.ChainID, suite.chainB.CurrentHeader.Height, trustedHeight, suite.chainB.CurrentHeader.Time, suite.chainB.Vals, suite.chainB.NextVals, trustedVals, suite.chainB.Signers)
		...
		},
		{
			...
			malleate: func() {
				...
				header = suite.chainB.CreateTMAttestorClientHeader(suite.chainB.ChainID, suite.chainB.CurrentHeader.Height+5, trustedHeight, suite.chainB.CurrentHeader.Time, bothValSet, suite.chainB.NextVals, trustedVals, bothSigners)
			},
			expPass: true,
		},
		{
			...
			malleate: func() {
				...
				header = suite.chainB.CreateTMAttestorClientHeader(suite.chainB.ChainID, suite.chainB.CurrentHeader.Height, trustedHeight, suite.chainB.CurrentHeader.Time, bothValSet, suite.chainB.NextVals, trustedVals, bothSigners)
			},
			expPass: true,
		},
		{
			...
			malleate: func() {
				...
				header = suite.chainB.CreateTMAttestorClientHeader(suite.chainB.ChainID, suite.chainB.CurrentHeader.Height+1, trustedHeight, suite.chainB.CurrentHeader.Time, bothValSet, bothValSet, bothValSet, bothSigners)
			},
			expPass: false,
		},
		{
			...
			malleate: func() {
				...
				header = suite.chainB.CreateTMAttestorClientHeader(suite.chainB.ChainID, suite.chainB.CurrentHeader.Height+1, trustedHeight, suite.chainB.CurrentHeader.Time, altValSet, altValSet, trustedVals, altSigners)
			},
			expPass: false,
		},
		{
		...
		},
		{
			...
			malleate: func() {
				...
				header = suite.chainB.CreateTMAttestorClientHeader(ibctesting.ChainIDRevision1, 3, trustedHeight, suite.chainB.CurrentHeader.Time, suite.chainB.Vals, suite.chainB.NextVals, trustedVals, suite.chainB.Signers)
			},
			expPass: false,
		},
		{
			...
			malleate: func() {
				...
				header = suite.chainB.CreateTMAttestorClientHeader(suite.chainB.ChainID, int64(heightMinus1.RevisionHeight), trustedHeight, suite.chainB.CurrentHeader.Time, suite.chainB.Vals, suite.chainB.NextVals, trustedVals, suite.chainB.Signers)
			},
			expPass: false,
		},
		{
		...
		},
		{
			...
			malleate: func() {
				...
				header = suite.chainB.CreateTMAttestorClientHeader(suite.chainB.ChainID, suite.chainB.CurrentHeader.Height+1, trustedHeight, suite.chainB.CurrentHeader.Time.Add(-time.Minute), suite.chainB.Vals, suite.chainB.NextVals, trustedVals, suite.chainB.Signers)
			},
			expPass: false,
		},
		{
			...
			malleate: func() {
				...
				header = suite.chainB.CreateTMAttestorClientHeader(ibctesting.ChainID, suite.chainB.CurrentHeader.Height+1, trustedHeight, suite.chainB.CurrentHeader.Time, suite.chainB.Vals, suite.chainB.NextVals, trustedVals, suite.chainB.Signers)
			},
			expPass: false,
		},
		{
			...
			malleate: func() {
				...
				header = suite.chainA.CreateTMAttestorClientHeader(suite.chainB.ChainID, suite.chainB.CurrentHeader.Height+1, trustedHeight, suite.chainB.CurrentHeader.Time, suite.chainB.Vals, suite.chainB.NextVals, trustedVals, suite.chainB.Signers)
...

		{
			...
			malleate: func() {
				...
				header = suite.chainB.CreateTMAttestorClientHeader(suite.chainB.ChainID+"-1", suite.chainB.CurrentHeader.Height+5, trustedHeight, suite.chainB.CurrentHeader.Time, suite.chainB.Vals, suite.chainB.NextVals, trustedVals, suite.chainB.Signers)
			},
			expPass: false,
		},
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			tmHeader, err := path.EndpointA.Chain.ConstructUpdateTMClientHeader(path.EndpointA.Counterparty.Chain, path.EndpointA.ClientID)
			suite.Require().NoError(err)
			header = &ibctmattestor.Header{Header: tmHeader}
...
func (suite *TMAttestorTestSuite) TestUpdateState() {
	var (
		path               *ibctesting.Path
	...
	testCases := []struct {
	...
	}{
		{
			"success with height later than latest height", func() {
				tmHeader, ok := clientMessage.(*ibctmattestor.Header)
		...
		},
		{
			"success with duplicate header", func() {
				...
				tmClientMessage, err := path.EndpointA.Chain.ConstructUpdateTMClientHeader(path.EndpointA.Counterparty.Chain, path.EndpointA.ClientID)
				suite.Require().NoError(err)
				clientMessage = &ibctmattestor.Header{Header: tmClientMessage}
		...
		},
		{
			"invalid ClientMessage type", func() {
				clientMessage = &ibctmattestor.Misbehaviour{}
			},
			func() {},
			false,
		},
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			suite.coordinator.CommitBlock(suite.chainB)
			var err error
			clientMessage, err = path.EndpointA.Chain.ConstructUpdateTMAttestorClientHeader(path.EndpointA.Counterparty.Chain, path.EndpointA.ClientID)
			suite.Require().NoError(err)
			...
			if tc.expPass {
				...
				header := clientMessage.(*ibctmattestor.Header)
				expConsensusState := &ibctmattestor.ConsensusState{
					ConsensusState: &ibctm.ConsensusState{
						Timestamp:          header.GetTime(),
						Root:               commitmenttypes.NewMerkleRoot(header.Header.Header.GetAppHash()),
						NextValidatorsHash: header.Header.Header.NextValidatorsHash,
					},
				}
				...
				consensusState, found := suite.chainA.App.GetIBCKeeper().ClientKeeper.GetClientConsensusState(suite.chainA.GetContext(), path.EndpointA.ClientID, clienttypes.NewHeight(1, uint64(suite.chainB.GetContext().BlockHeight())))
				suite.Require().False(found)
...
func (suite *TMAttestorTestSuite) TestUpdateStateCheckTx() {
	path := ibctesting.NewPathWithTendermintAttestors(suite.chainA, suite.chainB, 0, 0)
	...
	createClientMessage := func() exported.ClientMessage {
		...
		suite.Require().NoError(err)
		return &ibctmattestor.Header{Header: header}
	}
	...
	ibctmattestor.IterateConsensusStateAscending(clientStore, getFirstHeightCb)
	...
	cs.UpdateState(ctx, suite.chainA.App.AppCodec(), clientStore, createClientMessage())
	...
	assertPrune := func(pruned bool) {
		...
		processTime, ok := ibctmattestor.GetProcessedTime(clientStore, pruneHeight)
		suite.Require().Equal(!pruned, ok)

		processHeight, ok := ibctmattestor.GetProcessedHeight(clientStore, pruneHeight)
		suite.Require().Equal(!pruned, ok)

		consKey := ibctmattestor.GetIterationKey(clientStore, pruneHeight)
...
func (suite *TMAttestorTestSuite) TestPruneConsensusState() {
	...
	ctx = path.EndpointA.Chain.GetContext()
	clientStore = path.EndpointA.Chain.App.GetIBCKeeper().ClientKeeper.ClientStore(ctx, path.EndpointA.ClientID)
	expectedProcessTime, ok := ibctmattestor.GetProcessedTime(clientStore, expiredHeight)
	suite.Require().True(ok)
	expectedProcessHeight, ok := ibctmattestor.GetProcessedHeight(clientStore, expiredHeight)
	suite.Require().True(ok)
	expectedConsKey := ibctmattestor.GetIterationKey(clientStore, expiredHeight)
	...
	processTime, ok = ibctmattestor.GetProcessedTime(clientStore, expiredHeight)
	...
	processHeight, ok = ibctmattestor.GetProcessedHeight(clientStore, expiredHeight)
	...
	consKey = ibctmattestor.GetIterationKey(clientStore, expiredHeight)
	suite.Require().Equal(expectedConsKey, consKey, "iteration key incorrectly pruned")
}

func (suite *TMAttestorTestSuite) TestCheckForMisbehaviour() {
	var (
		path          *ibctesting.Path
		clientMessage exported.ClientMessage
	)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				header, ok := clientMessage.(*ibctmattestor.Header)
				suite.Require().True(ok)
		...
		},
		{
			"invalid fork misbehaviour: identical headers", func() {
				...
				clientMessage = &ibctmattestor.Misbehaviour{
					Header1: &ibctmattestor.Header{Header: misbehaviourHeader},
					Header2: &ibctmattestor.Header{Header: misbehaviourHeader},
				}
			}, false,
		},
		{
		...
		},
		{
			...
			func() {
				...
				header.Header.Header.Time = header.GetTime().Add(-time.Hour)
			},
			true,
		},
		{
			...
			func() {
				...
				header.Header.Header.Time = header.Header.Header.Time.Add(time.Hour)
			},
			true,
		},
		{
			...
			func() {
				...
				clientMessage = &ibctmattestor.Misbehaviour{
					Header1: &ibctmattestor.Header{Header: header1},
					Header2: &ibctmattestor.Header{Header: header2},
				}
			},
			true,
		},
		{
			"valid time misbehaviour: not monotonically increasing time", func() {
				...
				clientMessage = &ibctmattestor.Misbehaviour{
					Header2: suite.chainB.CreateTMAttestorClientHeader(suite.chainB.ChainID, suite.chainB.CurrentHeader.Height+3, trustedHeight, suite.chainB.CurrentHeader.Time.Add(time.Minute), suite.chainB.Vals, suite.chainB.NextVals, trustedVals, suite.chainB.Signers),
					Header1: suite.chainB.CreateTMAttestorClientHeader(suite.chainB.ChainID, suite.chainB.CurrentHeader.Height, trustedHeight, suite.chainB.CurrentHeader.Time, suite.chainB.Vals, suite.chainB.NextVals, trustedVals, suite.chainB.Signers),
				}
			}, true,
		},
	}
...
func (suite *TMAttestorTestSuite) TestUpdateStateOnMisbehaviour() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			if tc.expPass {
				...
				suite.Require().Equal(frozenHeight, newClientState.(*ibctmattestor.ClientState).FrozenHeight)
			}
		})
	}
}
