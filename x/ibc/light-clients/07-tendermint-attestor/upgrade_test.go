...
func (suite *TMAttestorTestSuite) TestVerifyUpgrade() {
	...
	testCases := []struct {
		...
		},
		{
			...
			setup: func() {
				...
				upgradedConsState = &ibctmattestor.ConsensusState{
		...
		},
		{
			...
			setup: func() {
				...
				tmClient, _ := cs.(*ibctmattestor.ClientState)
		...
		},
		{
			...
			setup: func() {
				...
				// zero custom fields and store in upgrade store
				suite.chainB.App.GetUpgradeKeeper().SetUpgradedClient(suite.chainB.GetContext(), int64(lastHeight.GetRevisionHeight()), upgradedClientBz) //nolint:errcheck // ignore error for
		...
		},
		{
			...
			setup: func() {
				...
				suite.chainA.ExpireClient(ibctesting.UnbondingPeriod)
		...
		},
		{
			...
			setup: func() {
				...
				// zero custom fields and store in upgrade store
				suite.chainB.App.GetUpgradeKeeper().SetUpgradedClient(suite.chainB.GetContext(), int64(lastHeight.GetRevisionHeight()), upgradedClientBz)            //nolint:errcheck // ignore error for testing
				suite.chainB.App.GetUpgradeKeeper().SetUpgradedConsensusState(suite.chainB.GetContext(), int64(lastHeight.GetRevisionHeight()), upgradedConsStateBz) //nolint:errcheck // ignore error for testing
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			upgradedClientProof, err = path.EndpointA.GetProofWithAttestations(upgradedClientProof)
			suite.Require().NoError(err)

			upgradedConsensusStateProof, err = path.EndpointA.GetProofWithAttestations(upgradedConsensusStateProof)
			suite.Require().NoError(err)
